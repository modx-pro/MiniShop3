...
### [2025-11-24] Очистка: Удалены устаревшие чанки личного кабинета
...
**Требуется:** Пересобрать компонент через `php _build/build.php` для применения изменений
...
### [2025-11-23] Рефакторинг: Модульная структура layout личного кабинета
...
### [2025-11-23] Рефакторинг: Вынесение JavaScript форм авторизации в отдельный модуль
...
```
[ERROR] [Customer::getOrCreate] ⏯Starting customer retrieval/creation process. Token: 28b4352f...
...
[ERROR] [Customer::getOrCreate] SECURITY: Existing customer found - auto-login SKIPPED
...
if ($msCustomer) {
    ...
    $this->modx->log(
        modX::LOG_LEVEL_ERROR,
        "[Customer::getOrCreate] SECURITY: Existing customer found by email - auto-login SKIPPED (requires password authentication)"
    );
}
```
...
```
[ERROR] [Customer::getOrCreate] ⏯Starting customer retrieval/creation process
...
**Логи:**
```
⏯Starting customer retrieval/creation process
...
[WARN] RegisterService failed: Email already exists. Trying to find existing customer by email...
...
```
[INFO] ⏭Auto-login disabled (ms3_customer_auto_login_on_order=false)
[INFO] ⏭Auto-login disabled for existing customer
[INFO] ⏭Auto-register disabled (ms3_customer_auto_register_on_order=false)
```
...
### [2025-11-16] Архитектура: Консолидация миграций и упрощение кодовой базы
