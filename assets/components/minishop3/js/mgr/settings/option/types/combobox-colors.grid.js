ms3.grid.ComboboxColors = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'ms3-grid-combobox-colors';
    }
    ...
    ms3.grid.ComboboxColors.superclass.constructor.call(this, config);
};

// TODO: global var
var nameCell = '';

Ext.extend(ms3.grid.ComboboxColors, MODx.grid.LocalGrid, {
            ...
            renderer: function (value, cell, row) {
                ...
                if (value.indexOf('#') >= 0) {
                    ...
                    if (r >= 0x00 && g >= 0x00 && b >= 0x00) {
                        ...
                        return String.format('<span class="ms3-row-badge" style="border-bottom:2px dashed #f00;background-color:#fff;color:#f00;">{0}</span>', value);
                    }
                    if ((r > 0xbb && g > 0xbb && b > 0xbb) || r > 0xbb && g > 0xbb || g > 0xdd) {
                        text = '#000000';
                    }
                    color = value;
                    return String.format('<span class="ms3-row-badge" style="background-color:{0};color:{1};">{0}</span>', color, text);
                } else {
                    return value;
                }
            }
            ...
            renderer: function () {
                return String.format(
                    ...
                    <ul class="ms3-row-actions">\
                        <li>\
                            <button class="btn btn-default icon icon-trash action-red" title="{0}" action="removeColor"></button>\
                        </li>\
                    </ul>',
                    _('remove')
                );
    ...
    addColor: function () {
        ...
        //let rowi = this.getSelectionModel().getSelected().getChanges();
        let rowi = this.getSelectionModel().last;
...
Ext.reg('ms3-grid-combobox-colors', ms3.grid.ComboboxColors);