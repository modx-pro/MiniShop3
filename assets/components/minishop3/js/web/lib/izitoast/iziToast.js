...
(function (root, factory) {
  ...
  const ACCEPTSTOUCH = 'ontouchstart' in document.documentElement
  ...
  /**
	 ...
	 * @private
	 */
  const createInput = function (inputObj) {
    const allowedAttributes = ['type', 'id', 'class', 'name', 'style', 'placeholder', 'min', 'max', 'minlength', 'maxlength', 'min', 'disabled', 'readonly', 'required', 'size', 'multiple', 'value']
    const allowedInputs = ['select', 'checkbox', 'color', 'date', 'datetime-local', 'email', 'file', 'hidden', 'image', 'month', 'number', 'password', 'radio', 'range', 'search', 'tel', 'text', 'time', 'url', 'week']
    const wrap = document.createElement('div')
    let input

    if (inputObj.type && allowedInputs.indexOf(inputObj.type) !== -1) {
      const type = inputObj.type === 'select' ? 'select' : 'input'
