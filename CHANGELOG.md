...
### [2026-05-09] Сниппет msCart — не глушить вывод при `msorder` по умолчанию (#249)

**Изменение поведения:** при `?msorder=...` сниппет **`msCart`** больше не возвращает пустую строку по умолчанию — корзина загружается и отдаёт чанк / `return=data` (в т.ч. мини-корзина в общем layout).

Параметр **`suppressWhenMsOrder`** (`combo-boolean`): установите **`1` / `true`**, чтобы вернуть прежнее поведение «не выводить корзину на странице с номером заказа в URL» для конкретного вызова (например, второй блок корзины на странице «спасибо»).

**Миграция после обновления:** если на странице с `?msorder=...` был второй блок `msCart`, который раньше пропадал сам собой, и это было нужно, добавьте к этому вызову параметр `suppressWhenMsOrder=1` или скройте блок в шаблоне.
