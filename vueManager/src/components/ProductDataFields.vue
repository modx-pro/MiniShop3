...
import { fieldHtmlId } from '../utils/fieldHtmlId.js'
...
function isCheckboxXtype(xtype) {
  return xtype === 'xcheckbox' || xtype === 'checkbox'
}
    ...
    if (configResponse && configResponse.fields) {
      ...
      configResponse.fields.forEach(field => {
        ...
        if (productDataResponse && productDataResponse[fieldName] !== undefined) {
          ...
          fieldValues.value[fieldName] = isCheckboxXtype(field.xtype)
            ? normalizeCheckboxValue(value)
            : parseStructuredExtraFieldValue(field.xtype, value)
        } else {
          fieldValues.value[fieldName] = isCheckboxXtype(field.xtype) ? 0 : null
        }
      })
    } else {
          ...
          <div
            ...
            :class="[
              ...
              isFullWidthExtraFieldXtype(field.xtype)
                ? 'col-12'
                : `col-${field.width || 4}`,
              { 'field-checkbox': isCheckboxXtype(field.xtype) },
            ]"
          >
            <template v-if="isCheckboxXtype(field.xtype)">
            ...
            <template v-else>
              <label :for="fieldHtmlId(field.name)" class="field-label" :title="'[[+' + field.name + ']]'">
                {{ field.label }}
                <span v-if="field.required" class="required">*</span>
              </label>
