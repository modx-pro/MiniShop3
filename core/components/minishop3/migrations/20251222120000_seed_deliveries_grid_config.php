...
class SeedDeliveriesGridConfig extends AbstractMigration
{
    public function up()
    {
        ...
        if (!$this->hasTable($prefix . 'ms3_grid_fields')) {
            $this->output->writeln('<comment>Table ms3_grid_fields does not exist, skipping deliveries grid seed</comment>');
            return;
        }

        // Idempotency check
        $count = $this->fetchRow("SELECT COUNT(*) as cnt FROM {$prefix}ms3_grid_fields WHERE grid_key = 'deliveries'");
        if ($count['cnt'] > 0) {
            $this->output->writeln('<comment>Deliveries grid fields already exist, skipping</comment>');
            return;
        }
    ...
    public function down()
    {
        ...
        $this->execute("DELETE FROM {$prefix}ms3_grid_fields WHERE grid_key = 'deliveries'");
    }
}
