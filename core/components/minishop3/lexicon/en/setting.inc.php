...
$_lang['setting_ms3_status_sent_desc'] = 'Order status to set when a shipment becomes shipped (if the transition is allowed). Also used as the shipped status for order lifecycle ports (default seed: 4).';
...
$_lang['setting_ms3_inventory_enabled_desc'] = 'Off by default: cart, checkout and payment behave as before and ignore stock. When on, MiniShop3 checks availability before allocating an order number, reserves stock on the New status (ms3_status_new), commits on Paid (ms3_status_paid), and releases on Canceled (ms3_status_canceled) if the order was not yet paid. A failed payment changes the order status only when ms3_payment_on_failed_status is set; it defaults to 0, so the order is left as is and keeps its reserve. Point ms3_payment_on_failed_status at the cancel status to release stock automatically. After the status row is saved, a failing msOnChangeOrderStatus plugin returns an error but does not revert status_id or inventory (#596). NULL stock counts as 0: fill stock before turning this on, or products with an empty stock cannot be ordered. Stock is one number per product. Order lines that share a product_id, including different option variants, draw from that same stock. Replace the ms3_inventory service to use an external warehouse. Multi-warehouse, lots and ERP sync are out of scope.';
...
$_lang['setting_ms3_payment_on_failed_status_desc'] = 'Order status ID applied when a payment attempt fails or is cancelled before paid. 0 leaves the order unchanged. Default is 0.';
