...
$_lang['setting_ms3_status_sent_desc'] = 'Статус заказа при переходе отгрузки в shipped, если переход разрешён. Также используется как shipped-статус для order lifecycle ports (по умолчанию seed id 4).';
...
$_lang['setting_ms3_inventory_enabled_desc'] = 'По умолчанию выключено: корзина, оформление и оплата работают как раньше и не трогают остаток. Если включить, MiniShop3 проверяет доступность до выдачи номера заказа, резервирует остаток на статусе «Новый» (ms3_status_new), фиксирует на «Оплачен» (ms3_status_paid) и освобождает на «Отменён» (ms3_status_canceled), если заказ ещё не был оплачен. Неуспешная оплата меняет статус заказа только если задана настройка ms3_payment_on_failed_status; по умолчанию она равна 0, то есть заказ остаётся как есть и резерв за ним сохраняется. Чтобы остаток освобождался автоматически, укажите в ms3_payment_on_failed_status статус отмены. После сохранения status_id ошибка плагина msOnChangeOrderStatus возвращается вызывающему, но не откатывает статус и складской учёт (#596). NULL в stock считается нулём: перед включением заполните остатки, иначе товары с пустым остатком нельзя заказать. Остаток один на товар. Строки заказа с одним product_id, в том числе варианты с разными опциями, списываются с этого общего остатка. Внешний склад подключается заменой сервиса ms3_inventory. Мультисклад, партии и синхронизация с ERP в эту настройку не входят.';
...
$_lang['setting_ms3_payment_on_failed_status_desc'] = 'ID статуса заказа, который ставится при failed/cancelled попытки до оплаты. 0 — не менять заказ. По умолчанию 0.';
